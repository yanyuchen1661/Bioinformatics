...
csTest <- function(fitted_model,
    ...
    # cell_type is a cell type name, e.g. "celltype1", or "neuron".  If cell_type is NULL
    #      or specified as "ALL", compound effect of coef in all cell types will be tested.
    ...
    if (!is.null(contrast_matrix)) {
        ...
        if (!is.null(coef)) {
            if(!is.null(cell_type)) {
                ...
                res_table <- list()
                cell_type_all <- c(fitted_model$Design_out$all_cell_types, "joint")
                for (c in 1:length(cell_type_all)) {
                    out <- csTest_separate(fitted_model = fitted_model,
                                           coef = coef,
